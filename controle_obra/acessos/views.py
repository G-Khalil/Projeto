...
def recognize_and_register_auto(request):
    ...
    try:
        ...
        if best_match and best_distance < 0.55:
            ...
            acesso.save()
        return JsonResponse({'sucesso': True, 'funcionario': best_match.nome, 'tipo': tipo, 'hora': agora.strftime('%H:%M'), 'foto_url': best_match.foto.url if best_match.foto else None})        return JsonResponse({'erro': 'Não reconhecido'}, status=404)
    except Exception as e: return JsonResponse({'erro': str(e)}, status=500)
